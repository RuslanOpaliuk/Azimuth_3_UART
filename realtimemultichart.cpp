...
void RealTimeMultiChart::drawChart(struct timeval current_time, uint16_t *data)
{

    quint64 tv_sec = current_time.tv_sec % 100;

    quint16 counter = 1;
    if (0 == current_time.tv_sec % 10)
    {
        counter *= 10;
        if (0 == current_time.tv_sec % 100)
        {
            counter *= 10;
            if (0 == current_time.tv_sec % 1000)
            {
                counter *= 10;
                if (0 == current_time.tv_sec % 10000)
                {
                    counter *= 10;
                    if (0 == current_time.tv_sec % 100000)
                    {
                        counter *= 10;
                        if (0 == current_time.tv_sec % 1000000)
                        {
                            counter *= 10;
                        }
                    }
    ...
    tv_sec *= 1000000000 * counter;

    quint64 tv_nsec = current_time.tv_usec;

    double currentTime = tv_sec + tv_nsec;
...
void RealTimeMultiChart::clearData()
{
    for (int i =0; i < sampleSize; i++)
    {
        m_timeStamps[i] = 0;
    ...
    const char *name, int color, Axis* xAxisScale, bool xAxisVisible)
{
    ...
    else if (m_currentIndex > 0)
    {
        ...
        c->xAxis()->setFormatCondition("align", 10);
    ...
    //    c->xAxis()->setLabelFormat("{value|P4}");
        ...
        c->xAxis()->setMinTickInc(initialFullRange);
    }
...
void RealTimeMultiChart::drawMultiChart(QChartViewer* viewer)
{
    ...
    m->addChart(0, 30 + chartHeight, drawXYChart(viewer, m_dataSeriesB, "Beta", 0x00ff00,
        xAxisScale, false));

    // The last chart displays the x-axis.
    m->addChart(0, 30 + chartHeight * 2, drawXYChart(viewer, m_dataSeriesC, "Gamma", 0x0000cc,
        xAxisScale,  true));
...
void RealTimeMultiChart::drawXYTrackLine(DrawArea* d, XYChart* c, int mouseX, bool hasXAxis)
{
    ...
    if (hasXAxis)
    {
        ...
        xlabel << "<*font,bgColor=000000*> " << c->xAxis()->getFormattedLabel(xValue, "P4")
            << " <*/font*>";
